...
export const ChatViewer: React.FC<ChatViewerProps> = ({ importId }) => {
  ...
  const messages = useLiveQuery(async () => {
    ...
    if (debouncedSearch) {
      collection = collection.filter((m) => m.rawText.toLowerCase().includes(debouncedSearch.toLowerCase()));
    }

    const offset = page * PAGE_SIZE;
    // Note: sorting logic remains same
    ...
    return (await collection.toArray()).slice(offset, offset + PAGE_SIZE);
  }, [importId, page, debouncedSearch]);

  // Count for pagination
  const totalCount = useLiveQuery(async () => {
    if (!debouncedSearch) {
      return db.messages.where("importId").equals(importId).count();
    }
    return db.messages
      .where("importId")
      .equals(importId)
      .filter((m) => m.rawText.toLowerCase().includes(debouncedSearch.toLowerCase()))
      .count();
  }, [importId, debouncedSearch]);

  const totalPages = Math.ceil((totalCount || 0) / PAGE_SIZE);

  return (
    <div className="card bg-base-100 border border-base-300/60 shadow-xl rounded-2xl flex flex-col h-[600px]">
